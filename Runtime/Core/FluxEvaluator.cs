...
namespace FluxFormula.Core
{
    ...
    internal readonly unsafe ref struct FluxEvaluator<TData, TOper, TProvider>
        where TData : unmanaged
        where TOper : unmanaged, Enum
        where TProvider : struct, IFluxDefinition<TData, TOper>
    {
        private readonly TProvider _provider;

        public FluxEvaluator(TProvider provider)
        {
            _provider = provider;
        }

        public TData Compute(ReadOnlySpan<Instruction> code)
        {
            ...
            fixed (Instruction* pBase = code)
            {
                for (int ip = 0; ip < code.Length; ip++)
                {
                    ...
                    {
                        regsPtr[inst->Dest] = _provider.Compute(operByte, *inst, registers);
