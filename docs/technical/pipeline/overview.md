...
字符串 / Token[]
  ...
  │   内部: 构建 FluxJITInjector（JIT 路径）或 FluxInjector（解释器路径）+ JIT 委托编译（IL 优先 → Expression 回退 → 解释器兜底）
...
除主四阶段管线外，以下子系统提供编译期代码生成、运行时渐进求值和持久化能力：

- **[LiteralScanner Source Generator](./literal-scanner-sg.md)**：编译期从 attribute 声明生成专用 Span 字面量扫描器。`FluxLexer` 在构造时优先使用生成的扫描器。
- **[Blob 注册表](../blob-registry.md)**：预编译公式的二进制分发管线。`IFluxBlobRegistry` + `BlobRegistryGenerator` + `FluxBlob.Load/Unload` 可加模式支持多 mod。
- **[VFF 持久化格式](../vff-format.md)**：`ChainLink[]` 的持久化形态。按 `DualHash64` 引用 blob 中的公式，支持参数覆写和递归组装。
- **[柯里化求值器](./curry-evaluator.md)**：函数式 State→State 渐进绑定。与 `FluxEvaluator`（热路径）和 `FluxStepEvaluator`（单步调试）构成三态求值架构。
- **[JIT 注入器](./jit-injector.md)**：`FluxJITInjector`（2 字段零分支）独立类型，v5.7.1 从 `FluxInjector` 拆分。

## 下一步

- [数据注入器](./injector.md)：Set/SetIndex 的内部机制
- [JIT 注入器](./jit-injector.md)：FluxJITInjector 热路径注入
