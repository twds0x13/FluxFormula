...
答案是四层 Source Generator 管线：开发者通过 attribute 声明数据格式，`IIncrementalGenerator` 在编译期将其翻译为专用 C# Span 扫描器，注入 `LiteralScanners` partial class。运行时零反射、零装箱、零委托分配。
...
| Attribute | 目标 | 语义 |
|-----------|------|------|
| `[LiteralTemplate("...")]` | struct | 定义 struct 的字面量模板 |
| `[ExternalLiteralTemplate(typeof(X), "...")]` | assembly | 为第三方类型定义模板（不需修改原类型） |
| `[LiteralTypeAlias("Alias", "float")]` | assembly | 自定义类型别名，模板中可用别名替代完整类型名 |
| `[LiteralTag("tag")]` | enum 成员 | 枚举成员的字符串标签，自动生成 tag→enum 扫描器 |
...
模板依赖图构建与拓扑排序：当 struct A 的模板引用了 struct B，`BuildDependencyGraph` 构建有向图，检测循环依赖（FLX002），生成拓扑排序。`CodeEmitter` 按依赖顺序发射代码（被依赖者先发射）。
...
| Pipeline | 输入 | 过滤 | 输出 |
|----------|------|------|------|
| A | `[LiteralTemplate]` 标记的 struct | `StructDeclarationSyntax` | `StructInfo`（struct 名、字段、模板） |
| B | `[ExternalLiteralTemplate]` | 任意语法节点 | 与 Pipeline A 相同的 `StructInfo` |
| C | `[LiteralTypeAlias]` | assembly 级 attribute | 别名映射 |
| D | `[LiteralTag]` 标记的 enum 成员 | `EnumMemberDeclarationSyntax` | tag→enum 扫描器 |
...
| 代码 | 级别 | 描述 |
|------|------|------|
| FLX001 | Error | 模板解析错误（语法错误、非法类型名） |
| FLX002 | Error | 循环模板依赖（A 引用 B，B 引用 A） |
| FLX003 | Error | readonly struct 使用 LiteralTemplate（无法字段赋值） |
| FLX004 | Warning | 模板引用了无 LiteralTemplate 的非内置类型，对应字段被跳过 |

## 与 FluxLexer 的关系

`FluxLexer` 在构造时按优先级选择字面量扫描器：

1. `LiteralScanners.TryGetScanner<TData>()` — 如果存在 `[LiteralTemplate]` 定义
