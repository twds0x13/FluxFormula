...
**(d) Variable patterns**: Variable syntax defined through `VariablePatternRule`'s Prefix/Suffix. E.g., `["[", "]"]` matches `[atk]`, `["{var:", "}"]` matches `{var:damage}`. Variable names are recorded alongside token positions in `LexResult.VarNames`.
