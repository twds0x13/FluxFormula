...
| `LiteralScanner` | `LiteralScanner<TData>` | 字面量扫描器委托。有 `[LiteralTemplate]` 时自动生成（可选），否则需手动设置 |
...
| `WhitespacePattern` | `string` | （已废弃，未使用）当前实现使用 `char.IsWhiteSpace` 逐字符跳过空白，不可替换。默认 `@"\s+"` |
