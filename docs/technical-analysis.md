...
与 v1.0.0 相比，v1.3.2 新增了 FluxLexer 层（字符串词法分析）和 FluxPlatform（JIT 平台降级控制）。
...
| # | 原问题 | 解决方案 | 位置 |
|---|--------|----------|------|
| 1 | JIT 在 IL2CPP/AOT 平台崩溃 | `FluxPlatform` + `Instantiate()` try-catch 自动降级 | `FluxEvaluator.cs:9` / `FluxAssembler.cs:97` |
| 2 | `Connect()` 无 Count=0 保护 | 卫语句: `if (this.Count == 0) return next; if (next.Count == 0) return this;` | `FluxFormula.cs:63-64` |
| 3 | TOper 底层类型无校验 | `FluxFormula` 静态构造函数 `sizeof(TOper) != 1` 检查 | `FluxFormula.cs:36-47` |
| 4 | 无真实测试 | 77 个单元测试, 覆盖编译/解释器/JIT/错误传播/Connect/Lexer/持久化 | `Tests/` 目录 9 个测试文件 |
| 5 | 无 IFluxDefinition 示例 | `FloatMathDef` 完整示例 + `SmokeTest.cs` | `Tests/TestDefinition.cs` |
| 6 | 寄存器模型无文档 | VitePress 文档站完整覆盖核心概念/API/内部原理 | `docs/` 目录 |
| 7 | FluxBinder 残留 | `ToString()` 已更新为 `"FluxInjector<{TData}>"` | `FluxInjector.cs:172` |
| 8 | AOT 兼容性未声明 | 文档/FAQ 明确标注平台支持矩阵 | `docs/faq.md` |
| 9 | README 未实现 | 中英文双语 README 完成 | `README.md` / `README.en.md` |
...
| 3 | 性能 | `buffer.ToArray()` 在 JIT 路径可能存在不必要的复制 | `FluxAssembler.cs` |
| 4 | 健壮性 | 寄存器永不重用, 253 个寄存器在超长公式中可能耗尽 | `FluxCompiler.cs` |
| 5 | 健壮性 | `"Out of registers."` 错误信息不含上下文 | `FluxCompiler.cs` |
| 6 | 健壮性 | `Set()` JIT 模式下 `offset + _slotsPerData` 可能 int 溢出（风险极低） | `FluxInjector.cs` |
| 7 | 文档 | 部分内部算法仍缺源代码级注释（调车场、寄存器分配、R1 注入） | `FluxCompiler.cs` |
| 8 | 优化 | JIT Expression Tree 未缓存 — 重复 `Compile()` 会产生重复编译开销 | `FluxJITCompiler.cs` |
