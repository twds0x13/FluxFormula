...
FluxFormula 是五分包 monorepo。按需选择安装哪些包，用户只需装真正需要的模块。
...
| `com.twds0x13.fluxformula.addressables.unitask` | UniTask 异步 Addressables 加载 | Addressables + UniTask |
| `com.twds0x13.fluxformula.burst` | Unity Jobs + Burst 编译器加速 | Core + Unity.Burst + Unity.Collections |
...
| Unity + UniTask 异步加载 | Core + FluxFormula + Addressables + Addressables.UniTask |
| Unity + Burst 高性能求值 | Core + Burst |
